...
fn run_part_1(input: &str, connections_to_make: u32) -> u32 {
    ...
    junction_boxes.iter()
        .for_each(|point| {
            let circuit = Circuit { identity: point.clone(), junction_boxes: HashSet::from([point.clone()]) };
    ...
    for distance in &distances {
        ...
        count += 1;
        if is_same_circuit {
            println!("Skipping distance {distance:?} because {one:?} and {other:?} are part of the same circuit");
            continue;
        }

        merge_circuits(&mut circuits, &mut circuit_lookup, distance.first_box.clone(), distance.second_box.clone());
    }

    let circuits: Vec<&Rc<Circuit>> = circuits.iter().collect();

    let mut circuits: Vec<usize> = circuits.iter().map(|circuit| circuit.junction_boxes.len().clone()).collect();
...
fn run_part_2(input: &str) -> u64 {
    ...
    for distance in &distances {
        ...
        if circuits.len() == 2 {
            ...
            println!("The product of X coordinates of the two last two circuits to be connected is {result:?}");
            return result;
        }
...
fn merge_circuits(circuits: &mut HashSet<Rc<Circuit>>, circuit_lookup: &mut HashMap<JunctionBox, Rc<Circuit>>, first_point: JunctionBox, second_point: JunctionBox) {
    let first_circuit = circuit_lookup.get(&first_point).unwrap_or_else(|| panic!("Circuit lookup does not have an entry for point {:?}", first_point));
    let second_circuit = circuit_lookup.get(&second_point).unwrap_or_else(|| panic!("Circuit lookup does not have an entry for point {:?}", second_point));

    let joined_circuit = Circuit {
        identity: first_circuit.identity.clone(),
        junction_boxes: first_circuit.junction_boxes.union(&second_circuit.junction_boxes).cloned().collect()
    };
...
fn parse_junction_boxes(input: &str) -> Vec<JunctionBox> {
    ...
    for box_text in input.lines() {
        let coords: Vec<f64> = box_text.split(',').map(|coord| coord.parse::<f64>().unwrap()).collect();
        let junction_box = JunctionBox { x: coords[0] as u64, y: coords[1] as u64, z: coords[2] as u64 };
...
fn test_rc_multiple_maps() {
    ...
    let c_from_lookup = junction_box_circuit_lut.get(&hashed_point).unwrap().as_ref().to_owned();
